...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def status_updates(self, nodes):
        ...
        testnode = [n for n in self.table_model.nodes if n.name == "config.tables.ve"]
        if testnode:
            if len(testnode) > 0:
                if testnode[0].val and "col" in testnode[0].val:
                    self.table_editor_model.setNode(testnode[0])
